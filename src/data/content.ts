...
export const siteContent = {
  ...
  questions: [
    ...
    },
    {
      ...
      answers: [
        ...
        { id: 'beak-mask', label: 'Маску с клювом. Лицо закрыто, зато сразу понятно, кто пришёл', score: 0 },
  ...
  results: {
    lark: {
      title: 'Жаворонок',
      description: 'Ты из тех редких птиц, которые способны не просто проснуться к восьми, но и добровольно куда-то к этому времени прилететь. Ты успеваешь прожить полдня до того, как остальные выпьют свою первую чашку кофе. Твой маршрут начинается с прогулки с орнитологом и наблюдения за настоящими птицами. Дальше день пойдёт своим ходом, но лучшая его часть уже принадлежит тебе.',
      defaultRoute: 'lark',
      recommendationTitle: 'Ранний вылет',
    ...
    owl: {
      ...
      defaultRoute: 'owl',
      recommendationTitle: 'Отложенный вылет',
      recommendationText: 'Утро можно провести без подвигов. К стае ты присоединишься уже в рабочем совином состоянии.',
    },
  } satisfies Record<BirdType, { title: string; description: string; defaultRoute: Flock; recommendationTitle: string; recommendationText: string }>,
  routes: {
    lark: {
      label: 'Ранний вылет',
      items: [
    ...
    pigeon: {
      label: 'Свободный городской маршрут',
      items: [
        'Можно примкнуть к жаворонкам и *Ричарду* в их экстремально ранней прогулке',
        'А можно дать себе поспать и прилететь с совами',
        'Определись в перекличке ниже, для прогулки важно число птиц',
      ],
    },
    owl: {
      label: 'Отложенный вылет',
      items: [
        'Утро можно проспать целиком без угрызений совести',
        'Время прилёта выбираешь сам',
  ...
  dayPlan: [
    ...
    },
    {
      ...
      text: 'Если погода позволяет, отдыхаем на пляже, катаемся на сапах и всячески чиллим. Закуски для общего стола только приветствуются. Пляж на территории заказника «Западный Котлин», поэтому мангалы нельзя, но если очень аккуратно, то можно. Ориентируемся часа на два, но решаем по ситуации.',
    },
    {
      title: 'Новоселье',
  ...
  weatherScenarios: [
    {
      ...
      items: [
        ...
        'Если погода позволяет, после прогулки идём на пляж',
    ...
    },
    {
      ...
      items: [
        ...
        'Если днём погода улучшается, встречаемся в квартире в Кронштадте, немного тусим и гуляем',
        'После смотрим по ситуации: остаёмся в Кронштадте или едем на проспект Королёва, 64к1',
        'Если погода плохая весь день, снимаем студию, дурачимся, затем едем на Королёва',
  ...
  gifts: {
    title: 'Подарки',
    text: 'Если подарочное вдохновение не прилетело, здесь можно подглядеть в мои хотелки.',
    instructions: [
  ...
  confirmation: {
    ...
    lead: 'Теперь у тебя есть весь план. Тест определил твою внутреннюю птицу, но будильник за тебя не поставит — выбери, во сколько прилетишь.',
    options: [
    ...
    messageHint: 'Необязательно. Вопрос, комментарий или просто пара слов.',
    savedNo: 'Жаль, но записано. Если планы изменятся — вернись и поменяй ответ.',
    messagePlaceholder: 'Напиши здесь, если хочется',
  ...
  ui: {
    ...
    name: {
      ...
      hint: 'Имя понадобится, чтобы сохранить твой маршрут.',
      label: 'Имя гостя',
    ...
    dayPlan: {
      ...
      title: 'План дня',
      note: 'Совам: обязательного времени приезда нет. Можно присоединиться позже.',
    },
    weather: {
    ...
    notices: {
      ...
      declined: 'Записали: в этот раз тебя не ждём.',
      restarted: 'Тест начат заново, прежний ответ обновится без дубля.',
    },
    save: {
    ...
    finale: {
      ...
      invitation: 'Буду ждать тебя, {name}!',
